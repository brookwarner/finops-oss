...
// Emergency fund (cash buffer): progress toward N months of spend.
export function bufferLines(d) {
  const lines = [];
  if (!d.configured) {
    lines.push(`Emergency fund: not set up — target ~${money(d.target)} (${d.targetMonths} mo of spend).`);
  ...
  lines.push(
    d.shortfall > 0
      ? `  ${cover} of ${d.targetMonths} mo spend covered · ${money(d.shortfall)} short`
      : `  ${cover} of ${d.targetMonths} mo spend covered · fully funded`,
  );
  return lines;
}
