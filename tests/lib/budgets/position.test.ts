...
describe("computePosition", () => {
  ...
  it("folds expectedInflow into income.expected on top of the plan baseline", () => {
    // Irregular income: small recurring plan ($470) + a logged invoice ($8,000)
    // due this cycle. Projected In = max(actual, plan) + expectedInflow.
    const p = computePosition({
      ...base, // day 6 of 30
      categoryKind: kinds([["inc", "income"], ["food", "monthly_cap"]]),
      budgets: [{ kind: "income", monthly_target: 470, name: "Partner ECE Income" }],
      txns: [
        { amount: 292, category_id: "inc", occurred_at: "2026-06-25T00:00:00" }, // actual so far
        { amount: -120, category_id: "food", occurred_at: "2026-06-22T00:00:00" }, // projected = 600
      ],
      expectedInflow: 8000,
    });
    // max(actual 292, plan 470) + invoiced 8000 = 8470
    expect(p.income.expected).toBe(8470);
    expect(p.income.expectedInflows).toBe(8000);
    // net.projected = 8470 − 600 = 7870
    expect(p.net.projected).toBe(7870);
  });

  it("defaults expectedInflows to 0 and leaves the projection plan-anchored", () => {
    const p = computePosition({
      ...base,
      categoryKind: kinds([["inc", "income"]]),
      budgets: [{ kind: "income", monthly_target: 470, name: "Partner ECE Income" }],
      txns: [{ amount: 292, category_id: "inc", occurred_at: "2026-06-25T00:00:00" }],
    });
    expect(p.income.expectedInflows).toBe(0);
    expect(p.income.expected).toBe(470); // max(292, 470) + 0
  });

  it("adds expectedInflow even with no income plan (pure contractor)", () => {
    // No income budget at all → baseline falls to recentRunRate (0 here), so the
    // projection is driven entirely by what's landed plus what's invoiced.
    const p = computePosition({
      ...base,
      categoryKind: kinds([["inc", "income"]]),
      budgets: [],
      txns: [{ amount: 1500, category_id: "inc", occurred_at: "2026-06-25T00:00:00" }],
      expectedInflow: 5000,
    });
    expect(p.income.expected).toBe(6500); // max(1500, runRate 0) + 5000
  });
...
describe("computePosition — income classification gates the plan baseline", () => {
  // day 6 of 30; no prior-window income → recentRunRate 0 unless stated.
  const periodStart = new Date("2026-06-20T00:00:00");
  const base = { periodStart, dayOfPeriod: 6, periodLength: 30 };
  const incCats = new Map([["inc", { kind: "income", group: null, name: "Contract" }]]);

  it("excludes an irregular-classified income budget from the plan baseline", () => {
    // Target lingers ($9,000) but it's classified irregular → must not anchor.
    const p = computePosition({
      ...base,
      categoryKind: incCats,
      budgets: [{ kind: "income", monthly_target: 9000, name: "Contract", incomeType: "irregular" }],
      txns: [{ amount: 1200, category_id: "inc", occurred_at: "2026-06-25T00:00:00" }],
    });
    expect(p.income.planned).toBe(0);
    // Baseline 0 (income budgets exist but none project forward) → expected = actual.
    expect(p.income.expected).toBe(1200);
  });

  it("projects an all-irregular household from actual + expected inflows only", () => {
    const p = computePosition({
      ...base,
      categoryKind: incCats,
      budgets: [{ kind: "income", monthly_target: 9000, name: "Contract", incomeType: "irregular" }],
    ...
    // max(actual 1200, baseline 0) + invoiced 5000 = 6200 — no phantom $9,000 plan.
    expect(p.income.expected).toBe(6200);
  });

  it("still anchors salary/recurring income budgets to their plan", () => {
    const p = computePosition({
      ...base,
      categoryKind: incCats,
      budgets: [
        { kind: "income", monthly_target: 8000, name: "Salary", incomeType: "salary" },
        { kind: "income", monthly_target: 500, name: "Partner", incomeType: "recurring" },
        { kind: "income", monthly_target: 9000, name: "Contract", incomeType: "irregular" },
      ],
      txns: [],
    });
    // Plan = salary 8000 + recurring 500; irregular 9000 excluded.
    expect(p.income.planned).toBe(8500);
    expect(p.income.expected).toBe(8500);
  });

  it("treats NULL/unclassified income as recurring (back-compat)", () => {
    const p = computePosition({
      ...base,
      categoryKind: incCats,
      budgets: [{ kind: "income", monthly_target: 4000, name: "Wage" }], // no incomeType
      txns: [],
    });
    expect(p.income.planned).toBe(4000);
    expect(p.income.expected).toBe(4000);
  });
});
