...
describe("shadowCommittedByCat", () => {
  ...
  it("seeds postings from the most recent cycle, not every occurrence", () => {
    const txns = [
    ...
    // April's payment is the PREVIOUS cycle of the same bill — projecting both
    // forward would double the monthly outflow.
    expect(bill).toMatchObject({ postings: [{ day: 8, amount: 250 }] });
  });

  it("seeds every distinct debit in a multi-bill category (the insurance case)", () => {
    const txns = [
      { amount: -272.66, category_id: "c1", occurred_at: "2026-05-20T00:00:00Z", merchant: "FidelityLife" },
      { amount: -150, category_id: "c1", occurred_at: "2026-05-20T00:00:00Z", merchant: "AA Insurance" },
      { amount: -5, category_id: "c1", occurred_at: "2026-05-20T00:00:00Z", merchant: "Sovereign" },
      // last month's copies of the same three debits — must not stack on top
      { amount: -272.66, category_id: "c1", occurred_at: "2026-04-20T00:00:00Z", merchant: "FidelityLife" },
      { amount: -150, category_id: "c1", occurred_at: "2026-04-20T00:00:00Z", merchant: "AA Insurance" },
      { amount: -5, category_id: "c1", occurred_at: "2026-04-20T00:00:00Z", merchant: "Sovereign" },
    ];
    const bill = shadowCommittedByCat({
      txns,
      categoryKind: kind([["c1", "ap_amortised", "Insurance"]]),
    ...
    expect(bill).toMatchObject({ postings: [{ day: 20, amount: 427.66 }] });
  });
});
