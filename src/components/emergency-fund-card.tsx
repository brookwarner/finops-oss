...
/** Emergency-fund progress: balance vs an N-months-of-spend target. Renders
 *  a setup prompt until an account is designated; nothing when there's no
 *  spend signal to size a target from. Server component (read-only). */
    ...
    return (
      <div className="rounded-card bg-surface p-5 shadow-card">
        ...
        <p className="mt-1.5 text-[13.5px] leading-snug text-ink-muted">
          ...
          <span className="font-semibold text-ink">{nzd(fund.target)}</span> (≈ {nzd(fund.monthlySpend)}/mo spend).
        </p>
    ...
    <div className="rounded-card bg-surface p-5 shadow-card">
      ...
      <div className="mt-2 flex items-baseline justify-between text-[12px] tabular-nums">
        <span className="text-ink-muted">
          {cover} of {fund.targetMonths} mo spend covered
        </span>
      ...
      <p className="mt-2.5 text-[11px] leading-snug text-ink-faint">
        Target = {fund.targetMonths} months × your total spend (≈ {nzd(fund.monthlySpend)}/mo).
      </p>
    </div>
  );
}
