...
/**
 * The balance/holdings refresh (balances, holdings, net-worth snapshot,
 * subscriptions, LLM categorisation fallback). Named "nightly" historically;
 * now runs every 4h (6×/day — see vercel.json) alongside the poll cron for
 * fresher data. Idempotent: balances/holdings upsert and the net-worth snapshot
 * upserts on (household_id, snapshot_date), so intraday re-runs overwrite the
 * same day rather than accumulating. The only per-run cost is the LLM
 * categorisation fallback, which is batch-capped, only touches uncategorised
 * rows, and learns rules so the same row isn't re-suggested.
 *
 * Lives in lib (not the route module) so both the `/api/cron/nightly` GET
 * handler and the user-facing manual-sync route (`/api/sync`) can call it —
 * Next.js forbids non-HTTP exports from a route file.
 */
