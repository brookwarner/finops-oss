...
// Intraday Akahu polling (every 4h, 6×/day — see vercel.json). Replaces the
// (now-removed) webhook+drain pipeline. Idempotent: fixed window + upsert +
// mark-and-sweep reconcile, so re-running mid-day never double-counts.
//
// Window strategy:
