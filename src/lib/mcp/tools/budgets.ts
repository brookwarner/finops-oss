...
import { setBudgetTarget, createBudget, BUDGET_KINDS } from "@/lib/budgets/write";
...
export const budgetTools: ToolDef[] = [
  ...
  defineTool(
    "create_budget",
    `Create a net-new budget. Attaches to an existing category if its name matches exactly (case-insensitive) and it has no budget yet, else creates the category too. Refuses (error: "already-has-budget") if a budget already exists for that category — use set_budget_target to change an existing one instead. kind must be one of: ${BUDGET_KINDS.join(", ")} — monthly_cap for an ordinary recurring cap, reserve for a sinking fund that accrues monthly and is drawn down on lumpy/seasonal spend (e.g. firewood, car rego, holidays), ap_amortised for a bill paid by automatic payment, income for an income source, savings for a contribution goal (money moved OUT to a saver, not spend). monthlyTarget is the per-month $ amount — for a reserve covering a known annual cost, pass annual/12. group is an optional display group (e.g. "Maintenance", "Discretionary"); left blank on an existing category it keeps that category's current group.`,
    createBudgetSchema.shape,
    async (args: { category: string; kind: string; monthlyTarget: number; group?: string }, extra) => {
      const supabase = createSupabaseServiceClient();
      const hid = householdId(extra);
      const r = await createBudget({
        supabase, householdId: hid, categoryName: args.category,
        kind: args.kind, monthlyTarget: args.monthlyTarget, group: args.group,
      });
      if (!r.ok) {
        if (r.reason === "already-has-budget") {
          return text({ error: r.reason, existingTarget: r.existingTarget });
        }
        return text({ error: r.reason });
      }
      return text({
        category: args.category, categoryCreated: r.categoryCreated,
        kind: r.kind, monthlyTarget: r.monthlyTarget, group: r.group,
      });
    },
  ),
];
