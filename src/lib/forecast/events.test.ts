...
describe("projectCommittedEvents", () => {
  const budgets: CommittedBudget[] = [
    { categoryId: "mortgage", kind: "ap_amortised", monthlyTarget: 1210, postings: [{ day: 21, amount: 1225 }], spendClass: "essential" },
    { categoryId: "rates", kind: "reserve", monthlyTarget: 300, postings: [], spendClass: "essential" },
  ];
  ...
  it("projects every posting of a multi-debit category, not just one", () => {
    // "Insurance" pays five direct debits on the 20th and a sixth on the 21st;
    // all of them leave the account, so all of them must be projected.
    const multi: CommittedBudget[] = [{
      categoryId: "insurance",
      kind: "ap_amortised",
      monthlyTarget: 500,
      postings: [{ day: 20, amount: 562.66 }, { day: 21, amount: 80.38 }],
      spendClass: "essential",
    }];
    const events = projectCommittedEvents(multi, new Date("2026-06-01"), 30);
    expect(events.map((e) => [e.date, e.delta])).toEqual([
      ["2026-06-20", -562.66],
      ["2026-06-21", -80.38],
    ]);
  });

  it("skips a zero-amount bill rather than emitting a $0 event", () => {
    const zero: CommittedBudget[] = [
      { categoryId: "dormant", kind: "ap_amortised", monthlyTarget: 0, postings: [], spendClass: "essential" },
    ];
    expect(projectCommittedEvents(zero, new Date("2026-06-01"), 30)).toEqual([]);
  });
});
...
describe("deriveEvents", () => {
  it("composes income, committed, and variable events into one sorted list", () => {
    const events = deriveEvents({
      ...
      incomeFallback: null,
      committed: [
        { categoryId: "mortgage", kind: "ap_amortised", monthlyTarget: 1210, postings: [{ day: 21, amount: 1210 }], spendClass: "essential" },
      ],
...
describe("nextBillCluster", () => {
  ...
  it("picks the largest cluster, not the earliest bill", () => {
    ...
    expect(c).not.toBeNull();
    expect(c!.date).toBe("2026-07-21");     // heaviest day (display anchor)
  ...
  it("folds bills within the gap into one cluster and splits distant ones", () => {
    const events: ForecastEvent[] = [
      ...
      bill("2026-06-21", 3000),  // gap 1 -> same cluster, and the heaviest day
      bill("2026-06-27", 18),    // gap 6 -> separate cluster
    ];
    const c = nextBillCluster(events, now);
    expect(c!.date).toBe("2026-06-21");     // anchors to the heaviest day, not the start
    expect(c!.endDate).toBe("2026-06-21");
    expect(c!.count).toBe(2);
    expect(c!.amount).toBe(4000);
  });

  it("anchors to the heaviest day, ignoring a trivial charge that opens the cluster", () => {
    // A $0.42 service fee on the 17th chains into the 20/21 mortgage via the gap.
    // It must extend the span (endDate) but never become the headline date.
    const events: ForecastEvent[] = [
      bill("2026-07-17", 0.42, "Service Fee"),
      bill("2026-07-20", 870, "Rates+Power"),
      bill("2026-07-21", 3654, "Mortgage"),
      bill("2026-07-22", 19.42, "Insurance"),
    ];
    const c = nextBillCluster(events, now);
    expect(c!.date).toBe("2026-07-21");     // mortgage day — the real weight
    expect(c!.endDate).toBe("2026-07-22");  // span still runs to the last bill
  ...
  it("anchors to the nearest material cluster, not a marginally-bigger far one", () => {
    // Six near-identical monthly clusters across a long (game-plan) horizon; a
    // short February folds the next month's $5 tax debit in, making it $5 bigger.
    // A global-max pick would anchor the verdict ~8 months out (the reported bug);
    // earliest-material must anchor to July, the imminent stressor.
    const monthly = (m: string) => [
      bill(`2026-${m}-20`, 870, "Rates+Power"),
      bill(`2026-${m}-21`, 3654, "Mortgage"),
    ];
    const events: ForecastEvent[] = [
      ...monthly("07"), ...monthly("08"), ...monthly("09"),
      ...monthly("10"), ...monthly("11"), ...monthly("12"),
      // Feb cluster, $5 heavier (short-month fold of the 1-Mar tax debit).
      bill("2027-02-20", 870, "Rates+Power"),
      bill("2027-02-21", 3654, "Mortgage"),
      bill("2027-02-23", 5, "Taxes"),
    ];
    const c = nextBillCluster(events, now);
    expect(c!.date).toBe("2026-07-21");  // July's heaviest day, not Feb
    expect(c!.amount).toBe(4524);
  });

  it("skips an immaterial stray and anchors to the first real cluster", () => {
    const events: ForecastEvent[] = [
      bill("2026-07-01", 5, "Taxes"),          // stray: below the materiality floor
      bill("2026-07-20", 870, "Rates+Power"),
      bill("2026-07-21", 3654, "Mortgage"),
    ];
    const c = nextBillCluster(events, now);
    expect(c!.date).toBe("2026-07-21");
    expect(c!.count).toBe(2);
  });
});
