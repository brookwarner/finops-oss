...
describe("appendShadowCommitted", () => {
  const makeShadow = (name: string, monthlyAvg: number, day = 15, amount = 50): ShadowBill => ({
  ...
  it("appends an unbudgeted recurring ap_amortised category as a committed bill", () => {
    ...
    expect(result[0].postings).toEqual([{ day: 12, amount: 25.99 }]);
  });
