...
export async function computeForecast(args: ComputeForecastArgs): Promise<ForecastResult> {
  ...
  if (candidateIds.length) {
    ...
    const shadowTxnRes = await db.transactions
      .select("amount, category_id, occurred_at, merchant, description")
  ...
  const assumptions: ForecastAssumptions = {
    ...
    dailyBurn: periodLength > 0
      ...
      : 0,
    // One row per dated posting — a category with several direct debits shows each
    // of them, so a wrong seed is visible rather than hidden behind a single line.
    bills: committed.flatMap((b): ForecastAssumptions["bills"] =>
      b.postings.length
        ? b.postings.map((p) => ({ name: b.categoryId, day: p.day, amount: p.amount, source: "actual" as const }))
        : [{ name: b.categoryId, day: 1, amount: b.monthlyTarget, source: "target" as const }],
    ),
  };
