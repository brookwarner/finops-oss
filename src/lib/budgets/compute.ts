...
import { mapInflows, expectedInflowByCycleEnd } from "@/lib/cashflow/inflows";
...
export async function computeBudgets(args: ComputeArgs): Promise<BudgetComputeResult> {
  ...
  const [budgetsRes, uncatRes, catsRes] = await Promise.all([
    db.budgets
      .select("id, monthly_target, kind, category_id, categories(id, name, group, income_type)")
      .eq("active", true)
  ...
  const [pendingRes, rulesRes, receivablesRes] = await Promise.all([
    ...
    // Receivable manual-asset rows (contract invoices, refunds, claims) + their
    // per-inflow terms — the forward-looking income for the position projection.
    db.accounts.select(
      "name, type, balance_current, akahu_account_id, expected_inflows(likelihood, expected_date, pre_tax, tax_rate)",
    ),
  ]);
  if (!pendingRes.error) {
  ...
  // Forward-looking irregular income: net (after-tax) receivables due to land by
  // cycle end. Folded into the position projection so the "In" reflects what's
  // actually been invoiced/owed rather than an assumed salary plan. Likely-only
  // (uncertain receivables aren't bankable). Errors → 0 (projection degrades to
  // the plan baseline, never throws the whole budgets page).
  const expectedInflow = receivablesRes.error
    ? 0
    : expectedInflowByCycleEnd(mapInflows((receivablesRes.data ?? []) as any[]), period.end, now);
  ...
  const position = computePosition({
    ...
    budgets: budgets.map((b: any) => {
      ...
      return { kind: b.kind as string, monthly_target: Number(b.monthly_target), name: (cat?.name as string) ?? "", categoryId: b.category_id as string, incomeType: (cat?.income_type as string | null) ?? null };
    }),
