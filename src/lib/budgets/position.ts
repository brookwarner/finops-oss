...
export interface PositionArgs {
  ...
  /** Active budgets — used for the plan benchmark + self-heal exclusion. For
   *  income budgets, `incomeType` (categories.income_type) gates whether the
   *  target anchors the projection — see `projectsForward`. NULL/absent ⇒
   *  recurring (back-compat). */
  budgets: { kind: string; monthly_target: number; name: string; categoryId?: string; incomeType?: string | null }[];
  periodStart: Date;
  dayOfPeriod: number;
  periodLength: number;
  /**
  ...
  /**
   * Net (after-tax) sum of logged expected inflows (receivables — contract
   * invoices, refunds, claims) due to land by cycle end. This is how irregular
   * income enters the projection: rather than assume a fixed salary plan arrives,
   * the forward-looking "In" is what's actually landed plus what's specifically
   * been invoiced/expected. No overlap with settled or pending income — these are
   * receivables that haven't posted yet (once paid, the balance zeroes and they
   * drop out). Defaults to 0. Compute via `expectedInflowByCycleEnd`.
   */
...
export interface Position {
  income: {
    ...
    /**
     * Net (after-tax) expected inflows (receivables) folded into `expected` —
     * the invoiced/owed portion of the projected "In", as distinct from the
     * recurring plan baseline. 0 when none logged. Lets a surface show
     * "incl. $X invoiced" so an irregular-income projection is legible.
     */
...
export function computePosition(args: PositionArgs): Position {
  ...
  const expectedInflow = Math.max(0, toCents(args.expectedInflow ?? 0));
  ...
  // planned = the household's FORWARD-projecting income budget for a cycle (Σ
  // active income budgets whose category is salary/recurring). monthly_target is
  // per-cycle, consistent with the expense plan benchmark below. Income a member
  // has explicitly classified irregular/one_off (a contractor's lumpy invoices, a
  // one-time payout) is real when it lands but must NOT anchor the projection as
  // if it recurs — the same `projectsForward` rule the forecast and cashflow
  // engines apply (src/lib/income/classify.ts). NULL/unclassified ⇒ recurring
  // (back-compat: every stream was projectable before classification existed).
  const planned = budgets
    .filter((b) => b.kind === "income" && projectsForward(b.incomeType))
    .reduce((sum, b) => sum + Number(b.monthly_target), 0);
  const hasIncomeBudget = budgets.some((b) => b.kind === "income");
  // Anchor to the forward plan. With no forward plan, fall back to the trailing
  // average ONLY for a fresh household that has set no income budgets at all. If
  // the household HAS income budgets but classified them ALL irregular/one_off,
  // respect that — there is no plannable baseline, so income is projected purely
  // from what's landed plus logged expected inflows (baseline 0, not a run-rate
  // that would re-bake the lumps the classification just excluded).
  const incomeBaseline = planned > 0 ? planned : hasIncomeBudget ? 0 : recentRunRate;
  // Projected "In" = the recurring plan baseline (or what's already landed, if
  // that's higher) PLUS explicitly-logged expected inflows due by cycle end.
  // The additive term is what makes the projection honest for irregular income:
  // a contractor zeroes the salary plan, so the baseline shrinks to genuinely
  // recurring streams, and forward income comes from receivables actually
  // invoiced — not an assumed paycheck. No double-count: expectedInflow is
  // outstanding receivables, disjoint from incomeActual (already posted).
  const incomeExpected = Math.max(incomeActual, incomeBaseline) + expectedInflow;
