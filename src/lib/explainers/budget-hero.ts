...
export function explainPosition(position: Position): Explainer {
  ...
  // The income projection is the greater of actual and plan baseline, PLUS any
  // logged expected inflows (invoiced receivables) due by cycle end. Spell out
  // the invoiced term only when there is one, so the salaried case reads clean.
  const incomeBasis = income.expectedInflows > 0
    ? `the greater of actual ${money(income.actual)} and plan ${money(income.planned)}, plus ${money(income.expectedInflows)} invoiced`
    : `the greater of actual ${money(income.actual)} and plan ${money(income.planned)}`;

  const rows: ExplainerRow[] = [
    {
      line: `${signedMoney(net.projected)} projected by cycle end`,
      meaning: `Projected end-of-cycle net — expected income (${money(income.expected)}, ${incomeBasis}) minus projected spend (${money(expenses.projected)}).`,
    },
    {
    ...
    },
    {
      ...
      meaning: income.expectedInflows > 0
        ? `Income received so far, projecting to the full-cycle expectation — including ${money(income.expectedInflows)} of invoiced/owed receivables expected by cycle end (the hatched bar).`
        : "Income received so far, projecting to the full-cycle expectation (the hatched bar).",
    },
    {
