  ...
  async function applyCategory(transactionIds: string[], categoryId: string | null) {
    ...
    try {
      ...
      startTransition(() => {
        ...
        // Bust the Next CLIENT Router Cache. Every bottom-nav tab is a
        // `<Link prefetch>` (PrefetchKind.FULL), whose whole RSC payload Next
        // reuses without refetching for `staleTimes.static` — 300s by default.
        // The route handler's `revalidateHousehold` only busts the SERVER data
        // cache; only a Server Action signals the client. Without this, tabbing
        // away and back within 5 minutes re-renders the pre-write payload and
        // every row you just categorised reads "Uncategorised" again — the write
        // persisted, the screen just lied. Also keeps the nav's inbox badge honest.
        // Local `txns` state is untouched by the refresh, so the inbox's
        // deliberate grey-out-in-place (no list jump) still holds.
        router.refresh();
      });
