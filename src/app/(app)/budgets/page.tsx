...
import { getCachedBalances } from "@/lib/accounts/balances";
...
export default async function BudgetsPage({
  ...
  const [forecast, cashflow, historyMap, incomeHistory, burn, salaried, allocInput, balances] = await Promise.all([
    ...
    getCachedBalances(householdId).catch(
      () => null as Awaited<ReturnType<typeof getCachedBalances>> | null,
    ),
  ]);
  ...
  const orderedGroups = orderGroups(grouped.keys());
  ...
  return (
    <section className="pb-12">
      ...
      {/* Current balance per account, with an inline edit mode choosing which
          accounts appear (accounts.show_on_dashboard). */}
      {balances && <BalancesCard result={balances} />}
      ...
      {/* Filter/sort bar — sits directly above the budget list (rather than up by
          the page title) so the controls are next to what they actually reorder.
          flex-wrap drops the controls to their own row on narrow phones instead
          of forcing the whole page wider than the viewport (which also throws off
