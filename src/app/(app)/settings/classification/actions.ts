...
import { setSpendClass, setCategoryGroup } from "@/lib/spend/sources";
...
/**
 * File a spend category under a group (the headings the /budgets list sections by),
 * or clear it with an empty string. Same session→service-role write path as above.
 * Purely presentational — nothing in the engines keys off the group — so this is
 * safe to change at will.
 */
export async function setCategoryGroupAction(
  ...
  const householdId = await requireHouseholdId();
  const supabase = createSupabaseServiceClient();
  const res = await setCategoryGroup({ supabase, householdId, categoryId, group });
  if (!res.ok) throw new Error(res.reason);
  revalidateHousehold(householdId);
  revalidatePath("/settings/classification");
  revalidatePath("/budgets");
